...
HazardResult AccessContext::DetectHazard(const IMAGE_STATE &image, SyncStageAccessIndex current_usage,
    ...
    if (!SimpleBinding(image)) return HazardResult();
    // TODO: replace the encoder/generator with offset3D/extent3D aware versions
    subresource_adapter::ImageRangeGenerator range_gen(*image.fragment_encoder.get(), subres_range, offset, extent);
    ...
    for (auto binding_pair : bindings) {
        ...
        for (uint32_t i = index_range.start; i < index_range.end; ++i, ++array_idx) {
            ...
            if (descriptor->GetClass() == DescriptorClass::ImageSampler || descriptor->GetClass() == DescriptorClass::Image) {
                ...
                if (hazard.hazard) {
                    skip |=
                        LogError(img_view_state->image_view, string_SyncHazardVUID(hazard.hazard), "Hazard %s for %s",
                ...
                if (hazard.hazard) {
                    skip |=
                        LogError(buf_view_state->buffer_view, string_SyncHazardVUID(hazard.hazard), "Hazard %s for %s",
                ...
                auto hazard = context->DetectHazard(*buf_state, SYNC_TRANSFER_TRANSFER_READ, range);
                if (hazard.hazard) {
                    skip |= LogError(buf_state->buffer, string_SyncHazardVUID(hazard.hazard), "Hazard %s for %s",
...
bool SyncValidator::DetectCommandBufferHazard(const CMD_BUFFER_STATE &cmd, VkPipelineBindPoint pipelineBindPoint) const {
    bool skip = false;
    ...
    for (const auto &set_binding_pair : pPipe->active_slots) {
        ...
        skip |= DetectDescriptorSetHazard(cmd, *descriptor_set, binding_req_map);
    }
    return skip;
}
...
bool SyncValidator::PreCallValidateCmdDispatch(VkCommandBuffer commandBuffer, uint32_t x, uint32_t y, uint32_t z) const {
    ...
    return DetectCommandBufferHazard(*cb_state, VK_PIPELINE_BIND_POINT_COMPUTE);
}
...
void SyncValidator::UpdateCommandBufferAccessState(const CMD_BUFFER_STATE &cmd, VkPipelineBindPoint pipelineBindPoint) {
    const auto last_bound_it = cmd.lastBound.find(pipelineBindPoint);
    if (last_bound_it == cmd.lastBound.cend()) {
        return;
    }
    ...
    for (const auto &set_binding_pair : pPipe->active_slots) {
        ...
        cvdescriptorset::PrefilterBindRequestMap reduced_map(*descriptor_set, set_binding_pair.second);
        const auto &binding_req_map = reduced_map.FilteredMap(cmd, *pPipe);
        UpdateDescriptorSetAccessState(cmd, CMD_DISPATCH, *descriptor_set, binding_req_map);
    }
}

void SyncValidator::PostCallRecordCmdDispatch(VkCommandBuffer commandBuffer, uint32_t x, uint32_t y, uint32_t z) {
    const auto *cb_state = Get<CMD_BUFFER_STATE>(commandBuffer);
    UpdateCommandBufferAccessState(*cb_state, VK_PIPELINE_BIND_POINT_COMPUTE);
}
